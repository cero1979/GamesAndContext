...
\title{Context-Dependent Benefit--Loss Games:\
Class Robustness and Strategic Stability}
...
\begin{quote}
What does it mean to transport the same strategic form from one social context to another while preserving, or failing to preserve, the benefit--loss class, strategic stability, and population-level consequences of its interactions?
\end{quote}
...
The local welfare function $W(x,y)=x+y$ used below is deliberately simple, but it allows the paper to separate two questions that are often conflated: whether a profile is strategically stable and whether it is socially desirable once both actor-side and affected-side payoffs are considered. This is important for the benchmark analysis, where one Nash equilibrium is malicious and welfare-dominated by another equilibrium.
...
We use $W=x+y$ only as a minimal diagnostic aggregate, not as a full welfare theory. It assumes interpersonal comparability and linear aggregation of actor-side and affected-side payoffs. We adopt it solely to separate, in the benchmarks, whether a profile is strategically stable from whether it is socially desirable once both sides are counted; richer or non-linear social welfare functionals could replace it without affecting the class-level results.
\end{remark}
...
The partition-based layer classifies profiles by the signs of their contextual payoff vectors. Since each valuation also gives a normal-form game, it is useful to separate two notions that are sometimes conflated: \emph{class stability} and \emph{strategic stability}. A profile can be intelligent without being a Nash equilibrium, and a malicious profile can be a Nash equilibrium if no player has a profitable unilateral deviation.

\begin{definition}[Class desirability]
\label{def:class-desirability}
A profile $\sigma$ in a context $C$ is \emph{benefit--loss desirable} (equivalently, \emph{class-desirable}) if $Q_C(\sigma)=\Iclass$, that is, if both the actor side and the affected side gain. Benefit--loss desirability is thus a purely class-based property of a single profile. It is distinct from \emph{welfare dominance}, which compares the aggregate $W=x+y$ of two profiles, and from \emph{Nash stability}, which concerns unilateral deviations. Whenever we contrast desirability with stability below, ``desirable'' means class $\Iclass$.
\end{definition}
...
This result is one of the main conceptual payoffs of the game-theoretic layer. Benefit--loss desirability and strategic stability are different dimensions. The business benchmark contains a malicious equilibrium: the profile is stable under unilateral deviations but relationally harmful to the affected side. Conversely, the sport benchmark contains an intelligent non-equilibrium: the profile benefits both sides but is strategically dominated for the acting side by constructive guidance. The strategic viability of $(P,U)$ in $C_B$ rests on the exact actor indifference $X_{C_B}(G,U)=X_{C_B}(P,U)$; Proposition~\ref{prop:knife-edge} shows that this equilibrium is a knife-edge in payoff magnitude, in contrast to the open-region robustness of its malicious \emph{class}.

\begin{proposition}[Class desirability and Nash stability are independent]
\label{prop:class-nash-independent}
In the benchmark family, benefit--loss desirability does not imply Nash stability, and Nash stability does not imply benefit--loss desirability.
\end{proposition}

\begin{proof}
First, consider the sport-team context $C_S$. Table~\ref{tab:payoff-transitions} gives $V_{C_S}(P,U)=(2,2)$, so $(P,U)$ is intelligent. However, Table~\ref{tab:br-ne} and Proposition~\ref{prop:benchmark-equilibria} show that the unique pure Nash equilibrium in $C_S$ is $(G,U)$. Equivalently, when the affected side plays $U$, the acting side obtains $3$ from $G$ and $2$ from $P$, so the acting side has a profitable unilateral deviation from $(P,U)$ to $(G,U)$. Thus an intelligent profile need not be Nash stable.

Second, consider the business context $C_B$. Table~\ref{tab:payoff-transitions} gives $V_{C_B}(P,U)=(2,-2)$, so $(P,U)$ is malicious. Proposition~\ref{prop:benchmark-equilibria} shows that $(P,U)$ is nevertheless a pure Nash equilibrium in $C_B$: the acting side is indifferent between $G$ and $P$ against $U$, and the affected side strictly prefers $U$ to $R$ against $P$. Thus Nash stability does not imply benefit--loss desirability.
\end{proof}
...
This proposition is the main welfare lesson of the benchmark. Strategic stability alone does not identify the socially preferable stable profile: in $C_B$, both $(G,U)$ and $(P,U)$ are equilibria, but only the former is both intelligent and welfare superior.
...
\subsection{Fixed points, stability, and the strategic knife-edge}
...
Two additional audits challenge the claims rather than merely regenerate the chosen example. First, we enumerate all $4^8=65{,}536$ $2\times2$ games whose eight payoff coordinates lie in $\{-2,-1,1,2\}$. Among these games there are 6,804 unordered pairs consisting of an intelligent and a malicious pure equilibrium. None shares an actor row, 2,688 share a column with zero failures of the dominance conclusion in Theorem~\ref{thm:dichotomy}(b), and 4,116 are diagonal. Of the diagonal pairs, 500 consist of strict equilibria with the intelligent profile welfare-dominating the malicious profile. This finite audit is not a proof, but it checks the implementation against every game in a nontrivial grid and searches explicitly for counterexamples.
...
The game-theoretic layer adds a second distinction. An intelligent profile need not be strategically stable, and a malicious profile may be a Nash equilibrium. In the benchmark business context, $(P,U)$ is malicious but stable under unilateral deviations. In the sport context, $(P,U)$ is intelligent but not a Nash equilibrium. Thus class desirability and strategic stability should be studied jointly rather than conflated.
...
The formal contributions include induced partitions of profile spaces, a characterization of class-preserving translations, a Hamming pseudometric for contextual sensitivity, and robustness theorems for perturbations and axis crossing. The strategic contribution shows that class stability and Nash stability are distinct, and the $2\times2$ configuration theorem separates a forced knife-edge regime from a possible robust diagonal regime. The computational audits challenge these claims through exhaustive finite enumeration, seeded perturbations, and convergence diagnostics.
