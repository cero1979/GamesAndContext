...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Computational contribution.} We give a reproducible exponential-replicator illustration, exhaustive square and rectangular game enumerations, and seeded continuous payoff perturbations. The audits test the finite-game theorem implementation, finite-horizon convergence, and the separation between class and equilibrium robustness; they are not empirical validation.
\end{enumerate}

After defining the contextual game, the paper states the two main finite-game theorems before developing translations, class sensitivity, and class robustness. The benchmark and dynamic sections then apply those results. The saturated feature matrices and their weight-space calculations are reported separately in Appendix~\ref{sec:features} so that the main argument does not depend on an unidentified parameterization.
...
\subsection{Equilibrium-preserving translations}
