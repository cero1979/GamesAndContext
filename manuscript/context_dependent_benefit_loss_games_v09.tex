...
\begin{abstract}
We study finite two-player games in which a common strategic form is evaluated by context-specific actor/affected-side payoff maps. The signs of these payoffs induce four relational classes---intelligent, hapless, malicious, and destructive---that apply to profiles rather than persons. We characterize class-preserving translations, define normalized Hamming sensitivity between contextual class maps, and derive exact robustness radii for class maps and complete pure-equilibrium sets in arbitrary finite two-player games. An intelligent and a malicious pure Nash equilibrium cannot share the actor's action. If they share the affected side's action, actor indifference is forced, the intelligent equilibrium Pareto- and welfare-dominates the malicious equilibrium, and coexistence is a knife-edge. Robust coexistence requires profiles that share neither action; in $2\times2$ games this is the diagonal configuration, and an explicit example has certified open-ball radius $3/4$. Three synthetic benchmarks illustrate how class robustness differs from equilibrium and dynamic robustness under an exponential-replicator update. Reproducible audits enumerate 390,625 square and 1,062,882 rectangular payoff-grid games and apply seeded continuous perturbations. The exhaustive audits find no counterexample, while perturbations preserve the business class map locally but destroy its original two-equilibrium set almost surely. The examples are formal benchmarks, not empirical estimates.
\end{abstract}
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Strategic contribution.} Two finite-game theorems give the exact radius preserving the complete pure-Nash set and characterize coexistence of intelligent and malicious equilibria. Sharing the actor's action is impossible; sharing the affected side's action forces an indifference knife-edge; robust welfare-dominated coexistence requires profiles sharing neither action. A $2\times2$ example realizes the robust case with a certified open-ball radius.
...
The distinction can be quantified exactly in any finite two-player game. For a profile $(i,j)\in A\times B$, collect all unilateral payoff advantages in
\[
\Delta_{ij}=
...
\begin{proof}
Each unilateral advantage is a difference of two payoff coordinates, so a sup-norm perturbation of radius $\delta$ changes it by less than $2\delta$. For an equilibrium profile, all advantages therefore remain nonnegative whenever $\delta<\tfrac12\min\Delta_{ij}$. For a non-equilibrium profile, at least one advantage is negative. If $d_*$ is a negative advantage with largest absolute value, then $d_*$ remains negative whenever $\delta<-d_*/2$; hence the profile remains outside the equilibrium set below $r_{ij}$. Taking the minimum over the finite profile set preserves the entire pure-equilibrium set.

For sharpness, choose a profile attaining the minimum. If it is an equilibrium, choose an advantage attaining $\min\Delta_{ij}$, lower its current payoff coordinate and raise the corresponding deviation payoff by any common amount strictly between $r_{ij}$ and $\varepsilon$; that advantage becomes negative. If it is not an equilibrium, increase the current payoff of each player by $r_{ij}$ and decrease that player's payoff at every profitable unilateral deviation by $r_{ij}$. These are distinct payoff coordinates across players, and every initially negative advantage $d$ becomes $d+2r_{ij}\ge0$. The profile therefore becomes a Nash equilibrium under a perturbation of norm $r_{ij}<\varepsilon$. In either case every enlargement beyond $\rho_{\NE}(G)$ permits a change in equilibrium membership.
\end{proof}
...
Proposition~\ref{prop:malicious-welfare-dominated} is a statement about one benchmark. We now show that it is an instance of a structural trichotomy for every finite two-player contextual benefit--loss game. The result explains why the business malicious equilibrium is fragile and identifies the only possible robust relative position. If $|A|=m$ and $|B|=n$, a game is identified with its payoff vector $v\in\R^{2mn}$, and a property is called \emph{robust} if it holds on an open neighborhood of $v$.

\begin{lemma}[Indifference forced by coexisting equilibria]
\label{lem:coexist}
Let a finite two-player contextual benefit--loss game have two distinct pure Nash equilibria. If they share the actor's action, the affected-side player is indifferent between the two affected-side actions at those profiles. If they share the affected side's action, the actor is indifferent between the two actor actions at those profiles.
\end{lemma}

\begin{proof}
Suppose $(a,b)$ and $(a,b')$ are equilibria with $b\ne b'$. Affected-side optimality at $(a,b)$ gives $Y_C(a,b)\ge Y_C(a,b')$, and optimality at $(a,b')$ gives the reverse inequality; hence equality holds. The shared-affected-action case is symmetric: actor optimality at $(a,b)$ and $(a',b)$ gives $X_C(a,b)=X_C(a',b)$.
\end{proof}

\begin{theorem}[Finite-game configuration theorem for intelligent and malicious equilibria]
\label{thm:configuration}
Let $G_C$ be a finite two-player contextual benefit--loss game, let $\sigma_I$ be an intelligent pure Nash equilibrium, and let $\sigma_M\ne\sigma_I$ be a malicious pure Nash equilibrium. Then exactly one of the following relative positions occurs.
\begin{enumerate}[label=(\alph*)]
\item \textbf{(Shared actor action: impossible.)} $\sigma_I$ and $\sigma_M$ cannot share the actor's action.
\item \textbf{(Shared affected-side action: forced knife-edge.)} If they share the affected side's action, then the actor is indifferent between the two actor actions, $X_C(\sigma_I)=X_C(\sigma_M)>0$, and $\sigma_I$ both Pareto-dominates and welfare-dominates $\sigma_M$. Every game supporting this fixed pair lies in the hyperplane $\{X_C(\sigma_I)=X_C(\sigma_M)\}\subset\R^{2mn}$. The existence of any such shared-action pair is therefore contained in a finite union of hyperplanes, has empty interior, and is never robust.
\item \textbf{(No shared action: the only possible robust regime.)} Otherwise the profiles differ in both players' actions. This relative position alone does not imply robustness. If both profiles are strict equilibria and $W(\sigma_I)>W(\sigma_M)$, their class, equilibrium, and welfare-dominance conditions are strict linear inequalities in $v$ and therefore persist on an open neighborhood. Such strict instances exist already in $2\times2$ games.
\end{enumerate}
Consequently, sharing no action is necessary but not sufficient for robust coexistence of an intelligent equilibrium and a welfare-dominated malicious equilibrium. In a $2\times2$ game this is exactly the diagonal configuration; sharing the affected-side action is necessarily a knife-edge.
\end{theorem}

\begin{proof}
(a) If $\sigma_I$ and $\sigma_M$ shared the actor's action, Lemma~\ref{lem:coexist} would give $Y_C(\sigma_I)=Y_C(\sigma_M)$. But $\sigma_I\in\Iclass$ forces $Y_C(\sigma_I)>0$ and $\sigma_M\in\Mclass$ forces $Y_C(\sigma_M)<0$, a contradiction.

(b) If they share the affected side's action, Lemma~\ref{lem:coexist} gives $X_C(\sigma_I)=X_C(\sigma_M)$. Both classes require a positive actor payoff, so this common value is positive. Since $Y_C(\sigma_I)>0>Y_C(\sigma_M)$ while the actor payoffs are equal, $\sigma_I$ Pareto-dominates $\sigma_M$, and
\[
W(\sigma_I)=X_C(\sigma_I)+Y_C(\sigma_I)>X_C(\sigma_M)+Y_C(\sigma_M)=W(\sigma_M).
\]
The defining equality confines the fixed pair to a hyperplane of $\R^{2mn}$. There are only finitely many profile pairs, so the set of games admitting any pair of this type is contained in a finite union of proper hyperplanes and has empty interior. Equivalently, arbitrarily small generic perturbations break all of the relevant actor ties. Thus shared-affected-action coexistence is not robust.

(c) By (a) and (b), the remaining relative position shares neither action. Weak equilibria can still rely on payoff ties, so this position alone is not sufficient. When both equilibria are strict, however, their finitely many best-response conditions are strict linear inequalities. The sign conditions defining $\sigma_I\in\Iclass$ and $\sigma_M\in\Mclass$, together with welfare dominance $W(\sigma_I)-W(\sigma_M)>0$, are also strict linear inequalities in $v$. A finite intersection of strict half-spaces is open. Proposition~\ref{prop:diagonal-robust} supplies a nonempty $2\times2$ instance, proving that the robust regime exists.
\end{proof}
...
The formal contributions include induced partitions of profile spaces, a characterization of class-preserving translations, a Hamming pseudometric for contextual sensitivity, and exact robustness radii for class maps and complete pure-equilibrium sets. The finite-game configuration theorem separates an impossible shared-actor-action case, a forced shared-affected-action knife-edge, and a potentially robust no-shared-action regime; in $2\times2$ games the last case is diagonal. Exhaustive square and rectangular enumeration, seeded perturbations, and convergence diagnostics challenge these claims computationally.
